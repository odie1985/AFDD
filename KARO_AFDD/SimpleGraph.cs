...
class SimpleGraph
{
   ...
   private void MapDataPoints( )
   {
      ...
      for (int offset = m_mapDataIndexPerPlot; offset < m_mapDataIndexPerPlot + drawPoint; offset++)
      {

         for (int i = 0; i < m_plotCount; i++)
         {
            m_dataPointBuffer[i][offset - m_mapDataIndexPerPlot].Y = (float)Math.Ceiling(YCordDividedRate *
               (m_YCordRangeMax - m_drawDataBuffer[(int)(m_plotCount * (offset + m_shiftCount) + i)]));
            m_dataPointBuffer[i][offset - m_mapDataIndexPerPlot].X = (float)Math.Round((offset - m_mapDataIndexPerPlot) * m_XCordDividedRate);
         }
      }
   }
